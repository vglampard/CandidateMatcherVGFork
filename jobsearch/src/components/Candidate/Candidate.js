...
const {email, skills, name, match, extra, availability} = candidate
  ...
  return (
    <div className={`flex gap-3 m-2 rounded bg-slate-200 p-2 drop-shadow ${match && match.length>0 ? "bg-green-300": "bg-red-100"}`} >
      ...
      <div className="flex gap-2 text-sm">
        ...
        {match.length > 0 &&  <div> <p>MATCH:</p> {match.map((match) => (
          <p>{match}</p>
        ))}</div>}
        <p></p>
      </div>
